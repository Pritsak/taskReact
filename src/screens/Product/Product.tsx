...
function Product() {
    ...
    return (
        <Container className={styles.product} maxWidth="sm">
            <img className={styles.product__image} src={data?.imageUrl} alt="product" />
            ...
            <Button onClick={() => setOpenModal(true)} style={{ position: "absolute", top: '20px', right: '20px' }} variant="contained" color="secondary">
                Update
            </Button>
            <div className={styles.add_comments_container}>
                <TextField value={comment} onChange={(e) => setComment(e.target.value)} placeholder='Comment...' color="secondary" fullWidth type='textarea' />
                <Button onClick={handleUpdateProduct} variant="contained" color="secondary">
                    Add Comment
                </Button>
            </div>
            <div className={styles.product__comments}>
                {data?.comments.map(elem => [
                    <Comment key={elem.id} date={elem.date} description={elem.description}/>
                ])}
            </div>
            <Modal
                open={openModal}
